...
/*
 * get_plt_entry() and friends build veneers with aarch64_insn_gen_*().
 * Those are declared in <asm/insn.h>, which reaches this file only by
 * way of <linux/ftrace.h> -> <asm/ftrace.h> on arm64. ARCH=um supplies
 * its own asm/ftrace.h, so the declarations do not arrive that way.
