...
/*
 * Sized like exec_regs (UM_GP_SLOTS): get_safe_registers() memcpy()s
 * the backend's full gp frame into this buffer. The arm64 marshal reads
 * internal slots past the ABI frame; a MAX_REG_NR buffer was overrun by
 * 3 slots, which clobbered using_seccomp on x86 (seccomp mode silently
 * disabled).
 */
