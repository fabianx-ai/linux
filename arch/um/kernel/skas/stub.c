...
restart_wait:
	 ...
	 * anything. When the kernel had not parked yet, the wake elided here
	 * is the syscall plus the voluntary context switch that dominates the
	...
	if (STUB_FUTEX_OWNER(futex_val) == FUTEX_IN_KERN) {
		...
		while (STUB_FUTEX_OWNER(futex_val) == FUTEX_IN_KERN) {
			 ...
			 * between the read and the wait impossible to lose:
