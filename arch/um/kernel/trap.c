 ...
 * to resume -- tens of microseconds per fault, dominated entirely by the two
 * voluntary context switches, not by handle_mm_fault. The stub mmap
 * transport, however, is already a batch: um_tlb_sync walks the marked PTE
 * range and queues one stub_syscall per page, and the whole queue is
 * executed in the *same* handoff that resumes the guest (see userspace() in
 * os-Linux/skas/process.c). So if we install K extra PTEs while we are here
 * anyway, K pages ride one handoff instead of costing K.
 *
 ...
 * random-access workload never pays for pages it will not touch; only a
 * stream that keeps faulting exactly past the previous window earns a bigger
 * one. 32 pages is where the source port measured the curve flattening.
 *
 ...
 * memory on a 16K guest than on a 4K one. The default scales the other way:
 * it is tied to the top of the ramp, so the full ramp is reachable at any
 * page size (128K on a 4K guest).
 */
